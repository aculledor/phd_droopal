 ...
 */
if (!empty($_SERVER['TRUSTED_HOST'])) {
  $settings['trusted_host_patterns'] = explode(',', $_SERVER['TRUSTED_HOST']);
}
 ...
 */
$features_enabled = [];
if (!empty($_SERVER['FEATURES_ENABLED'])) {
  $features_enabled = explode(' ', $_SERVER['FEATURES_ENABLED']);
}
foreach ($features_enabled as $feature_enabled) {
  $config['config_split.config_split.' . $feature_enabled]['status'] = TRUE;
}

/**
 * Google tag manager configuration.
 */
if (!empty($_SERVER['GOOGLE_TAG_CONTAINER_ID'])) {
  $config['google_tag.container.pnav']['tag_container_ids'] = explode(' ', $_SERVER['GOOGLE_TAG_CONTAINER_ID']) ?? [];
}

/**
 * Mail configuration.
 */
$config['symfony_mailer.mailer_transport.smtp']['configuration']['host'] = $_SERVER['MAIL_HOST'] ?? NULL;
$config['symfony_mailer.mailer_transport.smtp']['configuration']['pass'] = $_SERVER['MAIL_PASSWORD'] ?? NULL;
$config['symfony_mailer.mailer_transport.smtp']['configuration']['port'] = $_SERVER['MAIL_PORT'] ?? NULL;
$config['symfony_mailer.mailer_transport.smtp']['configuration']['user'] = $_SERVER['MAIL_USER'] ?? NULL;

if (isset($_SERVER['HTTP_X_FORWARDED_PREFIX'])) {
  $base_path = $_SERVER['HTTP_X_FORWARDED_PREFIX'];
}
