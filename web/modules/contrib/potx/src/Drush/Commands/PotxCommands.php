...
namespace Drupal\potx\Drush\Commands;
...
class PotxCommands extends DrushCommands {
   ...
   * @option themes Comma delimited list of themes to extract translatable strings from.
  ...
  public function potx($mode = NULL, array $options = [
    ...
    if (!empty($modules_option)) {
      ...
      foreach ($modules as $module) {
        ...
        $files = array_merge($files, _potx_explore_dir($path . '/', '*', $api_option, TRUE));
      }
    }
    elseif (!empty($themes_option)) {
      $themes = explode(',', $themes_option);
